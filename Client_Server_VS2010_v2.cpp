...
private:
	...
	static unsigned int WINAPI worker(void* p_to_this) {
		...
		srand((static_cast<uint32_t>(time(NULL)) << c->id_) + c->priority_);

		while (true) {
			{
				MutexGuard<Mutex> lock(c->exchange_point_.mutex_must_exit);
				if (c->exchange_point_.must_exit == true) {
					break;
				}
			}
			Sleep(rand() % MAX_MILLISECONDS_SLEEP);
...
private:
	...
	static unsigned int WINAPI worker(void* p_to_this) {
		...
		while (true) {
			{
				MutexGuard<Mutex> lock(s->exchange_point_.mutex_must_exit);
				if (s->exchange_point_.must_exit == true) {
					break;
				}
			}
...
int main() {
	...
	any_key = cin.get();
	{
		MutexGuard<Mutex> lock(exchange_point.mutex_must_exit);
